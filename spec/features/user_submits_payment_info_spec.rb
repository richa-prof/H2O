...
feature 'user submits payment info' do
  scenario 'for cart with BTMS item' do
  ...
  scenario 'and BTMS reservation fails' do
    ...
    create(:tour_locale, :pt_br, tour: tour, nome: 'Este Passeio Legal')
    ...
    expect(page).to have_content 'Este Passeio Legal'
    expect(page).to have_content 'ESCOLHER NOVA DATA E HORÁRIO'
    expect(page).to have_content 'REMOVER DO CARRINHO'
    expect(page).to have_content 'Carrinho: 555'
  ...
  scenario 'and hotel reservation fails' do
    create(:institutional, :with_pt_br_locale, tag: 'email_compra_online_reservas_confirmadas')
    ...
    create(:hotel_locale, :pt_br, hotel: cart_hotel.hotel, nome: 'Hotel Agradável')
    ...
    expect(page).to have_css "img[src*='cancel']"
    expect(page).to have_content 'Problemas com 1 ou mais solicitações de reservas. Veja os detalhes.'
    expect(page).to have_content 'Hotel Agradável'
    expect(page).to have_content 'ESCOLHER OUTRO PERÍODO'
    expect(page).to have_content 'REMOVER DO CARRINHO'
    expect(page).to have_content 'Carrinho: 555'
